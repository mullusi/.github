...
| `mullusi-govern-cloud` | Private Govern Cloud backend for `api.mullusi.com` governed evaluation and proof stamps | Private until launch review | Private-use notice recommended | CI and release-image workflow pass; production host, database, DNS, and secrets not verified here | Keep private; keep merge commits disabled; protect `main` when entitlement permits; deploy only after persistence and proof-stamp secrets are configured |
